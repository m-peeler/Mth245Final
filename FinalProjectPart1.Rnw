...
Finally, we will summarize the findings of our both of our models.

\begin{enumerate}[i.]
\item Model Output Summary 
<<>>=
summary(mod.accur.prelim.full)$adj.r.squared
AIC(mod.accur.prelim.full)
BIC(mod.accur.prelim.full)
@
...
We are 95\% confident that each coefficent falls within the ranges of their calculated confidence intervals listed in \ref{CIs for Partial Slope Parameters}.
\end{enumerate}
